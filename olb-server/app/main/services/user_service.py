...
def get_user_profile(user_id):
    board_count_sq = db.session.query(
                        db.func.count().filter(and_(UserBoard.user_id == user_id,
                                                    UserBoard.is_active))
                        .label("board_count"))\
        .subquery()

    match_count_sq = db.session.query(
                        db.func.count().filter(and_(Match.verified,
...
def get_user_boards(user_id, query_type):
    if(query_type is UserBoardQueryType.ADMIN):
        users_boards_sq = db.session.query(UserBoard.board_id)\
            .filter(and_(UserBoard.user_id == user_id,
                         UserBoard.is_active,
                         UserBoard.is_admin))\
            .subquery()
    ...
    ranked_boards_sq = db.session.query(
                        UserBoard.board_id,
                        UserBoard.user_id,
                        UserBoard.rating,
                        db.func.rank().over(
    ...
    full_boards_query = db.session.query(
                    ...
                    db.func.count().filter(and_(Match.winner_user_id == user_id, Match.verified)).over(
                            partition_by=ranked_boards_sq.c.board_id
                    ).label("wins"),
                    db.func.count().filter(and_(Match.winner_user_id != user_id, Match.verified)).over(
                            partition_by=ranked_boards_sq.c.board_id
                    ).label("losses"))\
        ...
        .filter(and_(ranked_boards_sq.c.user_id == user_id,
                     or_(ranked_boards_sq.c.user_id == Match.from_user_id,
                         ranked_boards_sq.c.user_id == Match.to_user_id,
                         Match.id == None))) # noqa E711 -- SQLAlchemy doesn't support "is"
