...
class BoardDto:
    ...
    board_profile_response = namespace.model(
        "board_profile_response",
        {
    ...
    board_members_response = namespace.model(
        "board_members_response", {"members": fields.List(fields.Nested(board_user))}
    )

    board_match = namespace.model(
        "board_match",
        {
            ...
            "submitter_result": fields.String(
                required=True, description="submitter's result of match {'Win', 'Loss', 'Draw'}"
            ),
            "receiver_result": fields.String(
                required=True, description="receiver's result of match {'Win', 'Loss', 'Draw'}"
            ),
        },
    )

    board_activity_response = namespace.model(
        "board_activity_response", {"matches": fields.List(fields.Nested(board_match))}
    )

    board_search = namespace.model(
        "board_search",
        {
    ...
    board_search_response = namespace.model(
        "board_search_response", {"search_result": fields.List(fields.Nested(board_search))}
    )

    create_board = namespace.model(
        "create_board",
        {
    ...
    create_board_response = namespace.model(
    ...
    edit_board_response = namespace.model(
        "edit_board_response",
        {
    ...
    invite_to_board = namespace.model(
        "invite_to_board",
        {
    ...
    invite_to_board_response = namespace.model(
        "invite_to_board_response",
        {
    ...
    join_board_response = namespace.model(
        "join_board_response",
        {
    ...
    remove_from_board = namespace.model(
        "remove_from_board",
        {
            "remove": fields.Boolean(
                required=True,
                description="Whether or not the user is being removed or just leaving\
                                                            {True = removing, False = leaving}",
            ),
            "board_id": fields.Integer(required=True, description="The id of the board being left"),
            "user_id": fields.Integer(
                required=False, description="The id of the user being removed (needed if remove is True)"
            ),
        },
    )

    remove_from_board_response = namespace.model(
        "remove_from_board_response",
        {
